...
# SCRIPT START
echo -n `TZ='Asia/Taipei' date +"[%m/%d %H:%M:%S] UTC+8 "` >> /var/log/cfupdater-6.log
echo "[Cloudflare DDNS] Check Initiated" >> /var/log/cfupdater-6.log
echo -n `TZ='Asia/Taipei' date +"[%m/%d %H:%M:%S] UTC+8 "` >> /var/log/cfupdater-6.log
# Seek for the record
...
if [ $ip == $old_ip ]; then
  echo "[Cloudflare DDNS] IP has not changed." >> /var/log/cfupdater-6.log
  exit 0
fi
...
case "$update" in
*"\"success\":false"*)
  >&2 echo -e "[Cloudflare DDNS] Update failed for $record_identifier. DUMPING RESULTS:\n$update" >> /var/log/cfupdater-6.log
  exit 1;;
*)
  echo "[Cloudflare DDNS] IPv6 context '$ip' has been synced to Cloudflare." >> /var/log/cfupdater-6.log;;
esac
EOF
