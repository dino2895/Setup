...
touch /var/log/cfupdater-v6.log
cat <<EOF > /usr/bin/cfupdater-v6
#!/bin/bash
...
# SCRIPT START
echo -n `TZ='Asia/Taipei' date +"[%m/%d %H:%M:%S] UTC+8 "` >> /var/log/cfupdater-v6.log
echo "[Cloudflare DDNS] Check Initiated" >> /var/log/cfupdater-v6.log
echo -n `TZ='Asia/Taipei' date +"[%m/%d %H:%M:%S] UTC+8 "` >> /var/log/cfupdater-v6.log
# Seek for the record
...
if [ $ip == $old_ip ]; then
  echo "[Cloudflare DDNS] IP has not changed." >> /var/log/cfupdater-v6.log
  exit 0
fi
# Set the record identifier from result
record_identifier=$(echo "$record" | grep -Po '(?<="id":")[^"]*' | head -1)
# The execution of update
update=$(curl -s -X PUT "https://api.cloudflare.com/client/v4/zones/$zone_identifier/dns_records/$record_identifier" -H "X-Auth-Email: $auth_email" -H "X-Auth-Key: $auth_key" -H "Content-Type: application/json" --data "{\"id\":\"$zone_identifier\",\"type\":\"AAAA\",\"proxied\":false,\"name\":\"$record_name\",\"comment\":\"$comment\",\"content\":\"$ip\"}")
# The moment of truth
case "$update" in
*"\"success\":false"*)
  >&2 echo -e "[Cloudflare DDNS] Update failed for $record_identifier. DUMPING RESULTS:\n$update" >> /var/log/cfupdater-v6.log
  exit 1;;
*)
  echo "[Cloudflare DDNS] IPv6 context '$ip' has been synced to Cloudflare." >> /var/log/cfupdater-v6.log;;
esac
EOF

chmod 700 /usr/bin/cfupdater-v6
...
[Service]
Type=oneshot
ExecStart=/usr/bin/cfupdater-v6
[Install]
